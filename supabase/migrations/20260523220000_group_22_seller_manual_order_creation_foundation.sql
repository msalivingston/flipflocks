-- Group 22: Seller Manual Order Creation Foundation
--
-- Scope:
-- - Adds a durable seller-facing RPC for creating manual/offline orders.
-- - Reuses existing customers, orders, order_items, inventory_items,
--   order idempotency, email notification outbox, and inventory activity log.
-- - Allows trusted seller-created orders to exceed currently listed inventory
--   while flooring inventory at zero and logging the override context.
--
...
-- - invoicing, quotes, buyer messaging, or CRM workflows
...
'Internal/system-facing source of the order. storefront is buyer checkout; seller_created/manual/phone/text/market/event are seller-created offline orders; imported is reserved for future import workflows. This should not clutter seller dashboards.';

create index if not exists orders_store_order_source_created_at_idx
on public.orders(store_id, order_source, created_at desc);
