-- Group 15: Checkout Delivery Foundation
--
-- Scope:
-- - Add buyer contact/address snapshot columns to orders.
-- - Require V1 checkout contact fields in trusted pay-at-pickup order creation.
-- - Update reusable customer contact/address details on successful checkout.
-- - Preserve existing inventory locking, idempotency, order numbering,
--   payment lifecycle, order item snapshots, and inventory decrement behavior.

alter table public.orders
add column buyer_address_line1_snapshot text,
add column buyer_address_line2_snapshot text,
add column buyer_city_snapshot text,
add column buyer_state_snapshot text,
add column buyer_postal_code_snapshot text,
add column buyer_country_snapshot text;

comment on column public.orders.buyer_address_line1_snapshot is
'Buyer contact address line 1 captured at checkout/order creation time. This is contact/location information, not payment or billing data. Historical snapshot should not depend on later customer edits.';

comment on column public.orders.buyer_address_line2_snapshot is
'Optional buyer contact address line 2 captured at checkout/order creation time. This is contact/location information, not payment or billing data. Historical snapshot should not depend on later customer edits.';

comment on column public.orders.buyer_city_snapshot is
'Buyer contact city captured at checkout/order creation time. Historical snapshot should not depend on later customer edits.';

comment on column public.orders.buyer_state_snapshot is
'Buyer contact state/region captured at checkout/order creation time. Historical snapshot should not depend on later customer edits.';

comment on column public.orders.buyer_postal_code_snapshot is
'Buyer contact postal code captured at checkout/order creation time. Historical snapshot should not depend on later customer edits.';

comment on column public.orders.buyer_country_snapshot is
'Buyer contact country captured at checkout/order creation time. Historical snapshot should not depend on later customer edits.';
