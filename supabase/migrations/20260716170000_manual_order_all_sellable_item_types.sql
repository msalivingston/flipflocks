-- Broaden manual order creation to every sellable storefront item type.
-- Reuses existing order item snapshot columns and inventory source tables.
begin;

drop function if exists public.seller_create_manual_order(uuid, text, jsonb, uuid, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, text, numeric, numeric, boolean, boolean);
...
'Trusted seller/admin RPC for atomically creating manual/offline orders. Supports inventory-backed items, custom non-inventory items, fulfillment option snapshots, delivery fees, idempotency, and optional existing transactional email notifications.';
