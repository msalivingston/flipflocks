-- Group 66B: Manual Order Creation Fixes
--
-- Fixes:
-- - Group 65 redefined seller_create_manual_order with search_path = public,
--   which made pgcrypto.digest unavailable on Supabase projects where pgcrypto
--   lives in the extensions schema.
-- - Seller manual orders need explicit over-available inventory overrides.
-- - Seller manual orders need simple custom/non-inventory line items.

create schema if not exists extensions;
create extension if not exists pgcrypto with schema extensions;

alter table public.order_items
  add column if not exists order_item_source text not null default 'inventory',
  add column if not exists custom_item_name_snapshot text;
...
alter table public.order_items
  ...
  drop constraint if exists order_items_custom_item_name_snapshot_not_empty_check,
...
alter table public.order_items
  add constraint order_items_order_item_source_check check (
    order_item_source in ('inventory', 'custom')
  ),
  add constraint order_items_custom_item_name_snapshot_not_empty_check check (
    custom_item_name_snapshot is null
    or length(trim(custom_item_name_snapshot)) > 0
  ),
  add constraint order_items_inventory_source_requires_inventory_check check (
  ...
  add constraint order_items_batch_type_snapshot_check check (
    batch_type_snapshot in ('live_animals', 'hatching_eggs', 'custom')
  );

comment on column public.order_items.order_item_source is
'Distinguishes inventory-backed order items from seller-entered custom manual order items.';

comment on column public.order_items.custom_item_name_snapshot is
'Seller-entered item name captured for custom manual order lines that do not reference inventory.';
...
begin
  ...
  select coalesce(
    sum((unit_price_override * quantity)::numeric(10, 2)),
    0
  )::numeric(10, 2)
...
'Seller-private order line detail projection for order detail and fulfillment screens, including manual custom items and age snapshots.';
