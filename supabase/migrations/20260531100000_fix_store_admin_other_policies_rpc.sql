-- Fix Store Admin Other Policies save support.
--
-- This forward migration is intentionally narrow. It makes the live database
-- tolerant of Store Admin sending other_policies by ensuring the column exists
-- and replacing the seller settings RPC allowed-key/update list.

alter table public.stores
add column if not exists other_policies text;
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'stores_other_policies_not_empty_check'
      and conrelid = 'public.stores'::regclass
  ) then
    alter table public.stores
    add constraint stores_other_policies_not_empty_check check (
      other_policies is null
      or length(trim(other_policies)) > 0
    );
  end if;
end;
$$;

comment on column public.stores.other_policies is
'Optional seller-managed public policy text for Store Admin when pickup and cancellation policy fields are not enough.';
...
'Seller-facing context RPC for UI bootstrapping. Includes Store Admin other_policies while omitting provider identifiers, admin hold details, suspension metadata, audit fields, and private billing provider IDs.';
...
'Seller/admin RPC for updating only seller-editable store settings, including Store Admin other_policies. It intentionally does not accept ownership, store_status, admin hold/suspension, billing/provider, audit, or system fields.';
