...
begin
  ...
  new_quantities as (
    select item_type, source_id, sum(quantity)::integer as quantity
    from pg_temp.edit_requested_items
    where source_id is not null and item_type <> 'custom'
    group by item_type, source_id
  )
  select
    coalesce(new_quantities.item_type, old_quantities.item_type) as item_type,
    coalesce(new_quantities.source_id, old_quantities.source_id) as source_id,
    coalesce(old_quantities.quantity, 0) as old_quantity,
    coalesce(new_quantities.quantity, 0) as new_quantity,
    coalesce(new_quantities.quantity, 0) - coalesce(old_quantities.quantity, 0) as quantity_delta
  from old_quantities
  full join new_quantities
    on new_quantities.item_type = old_quantities.item_type
   and new_quantities.source_id = old_quantities.source_id
  where coalesce(new_quantities.quantity, 0) <> coalesce(old_quantities.quantity, 0);

  if coalesce(p_change_inventory, false) then
    for v_record in
      select *
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'inventory_change_requested', coalesce(p_change_inventory, false),
  ...
  inventory_changed := coalesce(p_change_inventory, false);
