...
add column if not exists hero_subheading text not null
default 'Browse current availability and request pickup at checkout.';

update public.stores
set hero_subheading = 'Browse current availability and request pickup at checkout.'
where nullif(trim(hero_subheading), '') is null;

alter table public.stores
drop constraint if exists stores_hero_subheading_length_check;

alter table public.stores
add constraint stores_hero_subheading_length_check
check (length(trim(hero_subheading)) between 1 and 90);

comment on column public.stores.hero_subheading is
'Short seller-managed supporting line shown under the storefront hero headline.';
...
'Seller/admin RPC for updating only seller-editable store settings, including hero subheading and custom policy sections. It intentionally does not accept ownership, store_status, admin hold/suspension, billing/provider, audit, or system fields.';
...
'Slug-scoped public storefront home payload. Uses get_storefront_public_status for public availability, includes hero_subheading, respects disabled public selling modules in inventory summary counts, includes pickup_method, and returns public-safe hero crop/layout media fields.';
