...
-- Canceled orders should not reserve inventory. Keeping this in the seller
-- projection fixes existing canceled orders even if they were canceled before
-- the UI forced inventory restoration.
...
select
  ...
  case
    when o.order_status = 'canceled' then 0
...
'Seller-visible order item projection. remaining_unfulfilled_quantity is zero for canceled orders so canceled orders do not keep inventory reserved.';
...
begin
  ...
  where (
    ...
    or order_items.equipment_inventory_item_id in (
  ...
  and (
    ...
    or order_items.fulfilled_quantity > 0
  );

  if v_blocked_count > 0 then
    raise exception 'One or more selected inventory entries have active or fulfilled order history and cannot be permanently deleted.';
  end if;
  ...
  -- Canceled, unfulfilled order items are not meaningful order history for test
  -- inventory cleanup. Remove those child rows first so their FKs do not block
  -- deleting fake inventory entries.
  delete from public.order_items as order_items
  using public.orders as orders
  where orders.id = order_items.order_id
    and orders.store_id = order_items.store_id
    and orders.order_status = 'canceled'
    and order_items.fulfilled_quantity = 0
    and (
      order_items.inventory_item_id in (
        select deletable_items.id
        from pg_temp.deletable_inventory_items as deletable_items
...
'Trusted seller/admin RPC to permanently delete selected listing or equipment inventory rows that belong to the current seller/store. Blocks active or fulfilled order history; canceled, unfulfilled test order items are detached so fake inventory can be removed.';
