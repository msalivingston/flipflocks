...
-- The seller edit wrapper intentionally calculates a delta for every staged
-- inventory source. Scope that update explicitly so safe-update enforcement
-- accepts it without changing which valid staging rows are processed.
