-- Group 19: Notification Lifecycle Integration
--
-- Scope:
-- - Adds a small provider-agnostic helper for enqueueing transactional emails.
-- - Integrates email notification enqueueing into approved trusted order
--   lifecycle functions.
--
-- This group does not add:
-- - new notification tables
-- - delivery workers or Edge Functions
...
begin
  ...
  v_dedupe_key := p_notification_type || ':order:' || p_order_id::text;
...
'Trusted provider-agnostic helper for enqueueing transactional order email notifications. Normalizes blank recipient emails to no-op, validates V1 notification and recipient types, generates the approved dedupe key, and inserts a pending outbox row without sending email.';
...
'Trusted seller/admin RPC to mark a pending or open order fulfilled, preserving payment status and inventory while enqueueing the buyer fulfilled transactional email.';

comment on function public.cancel_order(uuid, text) is
'Trusted seller/admin RPC to cancel an eligible unpaid order, restore inventory quantities atomically, and enqueue the buyer canceled transactional email.';
