-- Processed Poultry typed order mutation support.
--
-- Replaces the pay-at-pickup creation RPC and lifecycle restoration RPCs so
-- listing_inventory, equipment_inventory, and processed_poultry_inventory are
-- validated, locked, decremented, snapshotted, and restored separately.
  ...
  if exists (
    ...
    where jsonb_typeof(raw_item.item) <> 'object'
       or (
        ...
        and not (
          raw_item.item ->> 'item_type' in ('listing_inventory', 'equipment_inventory', 'processed_poultry_inventory')
...
select
  ...
  oi.seller_breed_profile_id,
  oi.species_id,
  oi.species_name_snapshot,
  oi.species_slug_snapshot,
  oi.breed_display_name_snapshot,
  oi.breed_description_snapshot,
  oi.inventory_type_snapshot,
  oi.custom_inventory_label_snapshot,
  oi.batch_type_snapshot,
  oi.product_type_snapshot,
  oi.item_name_snapshot,
  oi.item_category_snapshot,
  oi.available_date_snapshot,
  oi.age_at_availability_days_snapshot,
  oi.unit_price_snapshot,
  oi.quantity,
  oi.fulfilled_quantity,
  oi.restored_quantity,
  greatest(oi.quantity - oi.fulfilled_quantity - oi.restored_quantity, 0) as remaining_unfulfilled_quantity,
  oi.line_subtotal,
  oi.created_at,
  oi.hatch_date_snapshot,
  oi.age_at_sale_days_snapshot,
  oi.order_item_source,
  oi.custom_item_name_snapshot
from public.order_items as oi
join public.orders as o
  on o.id = oi.order_id
 and o.store_id = oi.store_id
where public.owns_store(oi.store_id)
   or public.is_admin();
...
begin
  ...
  if exists (
    ...
    where oi.order_id = v_order.id
      and oi.store_id = v_order.store_id
      and (oi.fulfilled_quantity <> 0 or oi.restored_quantity <> oi.quantity)
  ) then
  ...
  where oi.order_id = v_order.id
    and oi.store_id = v_order.store_id
    and coalesce(oi.order_item_source, 'listing_inventory') in ('inventory', 'listing_inventory')
  group by ii.id, ii.quantity_available;
  ...
  where oi.order_id = v_order.id
    and oi.store_id = v_order.store_id
    and oi.order_item_source = 'equipment_inventory'
  group by ei.id, ei.quantity_available;
  ...
  where oi.order_id = v_order.id
    and oi.store_id = v_order.store_id
    and oi.order_item_source = 'processed_poultry_inventory'
  group by ppi.id, ppi.quantity_available;

  if exists (
    select 1
    from pg_temp.reinstate_order_items as roi
    where roi.quantity_available < roi.quantity
  ) then
    raise exception 'Insufficient inventory quantity available to reinstate this order.';
  end if;

  update public.inventory_items as ii
  set quantity_available = ii.quantity_available - roi.quantity
  from pg_temp.reinstate_order_items as roi
  where roi.item_type = 'listing_inventory'
    and ii.id = roi.inventory_item_id
    and ii.store_id = v_order.store_id;

  update public.equipment_inventory_items as ei
  set quantity_available = ei.quantity_available - roi.quantity
  from pg_temp.reinstate_order_items as roi
  where roi.item_type = 'equipment_inventory'
    and ei.id = roi.equipment_inventory_item_id
    and ei.store_id = v_order.store_id;

  update public.processed_poultry_inventory_items as ppi
  set quantity_available = ppi.quantity_available - roi.quantity
  from pg_temp.reinstate_order_items as roi
  where roi.item_type = 'processed_poultry_inventory'
    and ppi.id = roi.processed_poultry_inventory_item_id
    and ppi.store_id = v_order.store_id;
  ...
  select jsonb_build_object(
    ...
    coalesce(
      jsonb_agg(
        jsonb_build_object(
          'item_type', roi.item_type,
          'inventory_item_id', roi.inventory_item_id,
          'equipment_inventory_item_id', roi.equipment_inventory_item_id,
          'processed_poultry_inventory_item_id', roi.processed_poultry_inventory_item_id,
          'quantity_redecremented', roi.quantity
        )
        order by roi.item_type,
          coalesce(roi.inventory_item_id, roi.equipment_inventory_item_id, roi.processed_poultry_inventory_item_id)
      ),
      '[]'::jsonb
    )
  )
