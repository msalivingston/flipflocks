-- Archive seller pickup options without deleting records used by existing orders.

alter table public.store_pickup_options
...
comment on column public.store_pickup_options.archived_at is
'When the pickup option was archived. Active options keep this null.';
...
begin
  ...
  set
    ...
    archived_at = case
      ...
      else coalesce(v_option.archived_at, now())
    end
...
where stores.storefront_enabled = true
  ...
  and store_pickup_options.archived_at is null;

comment on view public.public_storefront_pickup_options is
'Buyer-facing active, non-archived pickup options for stores using the manual_options pickup method.';
...
begin
  ...
  if p_pickup_option_id is null then
    ...
    where store_pickup_options.id = p_pickup_option_id
      ...
      and (
        (
          store_pickup_options.is_active = true
          and store_pickup_options.archived_at is null
        )
        or (
          store_pickup_options.is_active = false
          and store_pickup_options.archived_at >= now() - interval '90 days'
        )
        or store_pickup_options.id = v_order.pickup_option_id
      );
...
create or replace function public.seller_edit_order_strict_wrapper(
  p_order_id uuid,
  p_items jsonb,
...
begin
  ...
  select
    ...
    case
      ...
      when order_items.order_item_source = 'hatching_egg_inventory'
        then order_items.hatching_egg_inventory_item_id
      else null
    end as source_id,
  ...
  if exists (
    ...
    join public.hatching_egg_inventory_items as hatching_items
      on hatching_items.id = requested.source_id
     and hatching_items.store_id = v_order.store_id
    where requested.item_type = 'hatching_egg_inventory'
      and requested.quantity < coalesce(hatching_items.minimum_order_quantity, 1)
  ) then
  ...
  if exists (
    ...
    where requested.item_type = 'hatching_egg_inventory'
      and (
        hatching_items.store_id <> v_order.store_id
        or hatching_items.visibility_status = 'archived'
        or hatching_items.moderation_status <> 'normal'
      )
  ) then
    raise exception 'One or more hatching egg items are not available for order editing.';
  end if;
  ...
  loop
    if v_record.item_type = 'inventory' then
      ...
      for update;
    elsif v_record.item_type = 'hatching_egg_inventory' then
      perform 1
      from public.hatching_egg_inventory_items as hatching_items
      where hatching_items.id = v_record.source_id
        and hatching_items.store_id = v_order.store_id
      for update;
    end if;
  ...
  loop
    if v_record.item_type = 'inventory' then
      ...
      select hatching_items.quantity_available
      into v_old_quantity
      from public.hatching_egg_inventory_items as hatching_items
      where hatching_items.id = v_record.source_id
        and hatching_items.store_id = v_order.store_id
      ...
      if v_record.quantity_delta > v_old_quantity then
        raise exception 'Insufficient hatching egg quantity available.';
      end if;

      v_new_quantity := v_old_quantity - v_record.quantity_delta;
  ...
  where requested.order_item_id is null
    and requested.item_type = 'hatching_egg_inventory'
  order by requested.line_number;
...
'Seller-private order management projection with fulfillment, refund, notification, pickup-option, archive, and contact data needed for dashboard order lists. Pickup labels prefer the current option record and fall back to the stored snapshot. Provider identifiers are intentionally omitted. Archived orders remain available in this broad history projection.';
...
'Seller-private dashboard projection listing pending/open orders that need attention. Pickup labels prefer the current option record and fall back to the stored snapshot.';
