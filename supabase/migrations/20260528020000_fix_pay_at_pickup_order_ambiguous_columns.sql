-- Fix ambiguous column references in public.create_pay_at_pickup_order(...).
--
-- The function returns a table with a store_id output column, which PL/pgSQL
-- treats as a variable. Qualify temp-table column references so checkout order
-- creation does not fail with "column reference store_id is ambiguous".
