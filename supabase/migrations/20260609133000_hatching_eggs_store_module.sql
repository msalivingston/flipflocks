-- Phase 1 seller inventory modules: Hatching Eggs.
--
-- Live Birds remains the always-on default workflow. This flag only controls
-- whether sellers see entry points for creating new hatching egg inventory.
-- Existing inventory, listings, orders, and history are not hidden or changed.

alter table public.stores
add column if not exists hatching_eggs_enabled boolean not null default false;

comment on column public.stores.hatching_eggs_enabled is
'Seller-controlled Store Admin setting for showing hatching egg creation entry points. It does not hide or modify existing inventory, listings, orders, or history.';
