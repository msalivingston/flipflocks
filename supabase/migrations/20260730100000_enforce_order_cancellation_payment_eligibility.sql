-- Block cancellation of online orders after payment is recorded.
-- All existing eligible-order cancellation behavior and the RPC signature remain unchanged.
...
begin
  ...
  if v_order.payment_method = 'stripe_checkout'
    ...
    raise exception 'Paid online orders cannot be canceled until Stripe refunds are supported.';
  end if;
  ...
  if v_restore_inventory then
    ...
    where oi.order_id = v_order.id
      ...
      and oi.hatching_egg_inventory_item_id is not null
      and oi.quantity - oi.fulfilled_quantity - oi.restored_quantity > 0
    order by hei.id
    for update of hei, oi;
    ...
    set quantity_available = hei.quantity_available + coi.quantity_to_restore
    from pg_temp.cancel_order_items as coi
    where coi.item_type = 'hatching_egg_inventory'
      and hei.id = coi.hatching_egg_inventory_item_id
      and hei.store_id = v_order.store_id;
    ...
    coalesce(
      jsonb_agg(
        jsonb_build_object(
          ...
          'hatching_egg_inventory_item_id', coi.hatching_egg_inventory_item_id,
...
'Trusted seller/admin RPC to cancel an eligible order, optionally restore inventory across all supported inventory sources, and optionally enqueue V1 buyer cancellation and seller-copy email notifications. Returns optional email queue results.';
