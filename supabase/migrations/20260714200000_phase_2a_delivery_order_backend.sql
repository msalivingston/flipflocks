-- Phase 2A backend delivery order support.
-- Adds order snapshot fields and a versioned order creation RPC only.
-- This does not change buyer checkout UI, public storefront loading, emails,
-- seller order displays, print views, Stripe, address fields, or Store Admin.

begin;

alter table public.orders
  add column if not exists fulfillment_method text not null default 'pickup',
  add column if not exists delivery_option_name_snapshot text,
...
alter table public.orders
  ...
  drop constraint if exists orders_delivery_fee_amount_nonnegative_check,
  drop constraint if exists orders_pickup_delivery_snapshot_check,
  drop constraint if exists orders_delivery_snapshot_required_check;

alter table public.orders
  add constraint orders_fulfillment_method_check check (
    fulfillment_method in ('pickup', 'delivery')
  ),
  add constraint orders_delivery_fee_amount_nonnegative_check check (
    delivery_fee_amount >= 0
  ),
  add constraint orders_pickup_delivery_snapshot_check check (
    fulfillment_method <> 'pickup'
    or (
      delivery_fee_amount = 0
      and delivery_option_name_snapshot is null
    )
  ),
  add constraint orders_delivery_snapshot_required_check check (
    fulfillment_method <> 'delivery'
    or nullif(trim(delivery_option_name_snapshot), '') is not null
  );

comment on column public.orders.fulfillment_method is
'Buyer-selected fulfillment method captured at order creation time. Phase 2A supports pickup and delivery while preserving existing pickup checkout behavior.';

comment on column public.orders.delivery_option_name_snapshot is
'Delivery option name captured at order creation time for delivery orders. Historical orders should not depend on later seller delivery option edits.';

comment on column public.orders.delivery_fee_amount is
'Delivery fee captured from the trusted store delivery option at order creation time. This fee is added to total_amount for delivery orders.';
