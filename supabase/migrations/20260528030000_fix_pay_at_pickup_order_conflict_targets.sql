-- Fix remaining ambiguous store_id conflict targets in public.create_pay_at_pickup_order(...).
--
-- The function has a store_id output column, so PL/pgSQL can treat unqualified
-- conflict-target column names as ambiguous. Refer to the primary-key
-- constraints instead of bare store_id column names.
