...
alter table public.media_links
drop constraint if exists media_links_hero_layout_check;

alter table public.media_links
add constraint media_links_hero_layout_check check (
  hero_layout is null or hero_layout in ('full', 'right')
);

comment on column public.media_links.hero_layout is
'Optional storefront hero layout for store hero media. full means the photo fills the hero preview; right means left-side text fade treatment. Null defaults to full.';
...
create or replace function public.seller_update_store_hero_layout(
...
declare
  ...
  v_layout text := lower(trim(p_hero_layout));
begin
  if v_layout not in ('full', 'right') then
    raise exception 'Unsupported hero layout';
  end if;
...
'Slug-scoped public storefront home payload. Uses get_storefront_public_status for public availability, respects disabled public selling modules in inventory summary counts, includes pickup_method, and returns public-safe hero crop/layout media fields.';

revoke all on function public.seller_update_store_hero_layout(uuid, text) from public;
revoke all on function public.get_public_storefront_home(text) from public;

grant execute on function public.seller_update_store_hero_layout(uuid, text) to authenticated;
