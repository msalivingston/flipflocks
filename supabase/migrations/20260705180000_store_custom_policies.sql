-- Add seller-managed custom policy sections without changing legacy policy fields.
-- Existing pickup_policy, cancellation_policy, and other_policies remain intact.

alter table public.stores
add column if not exists custom_policies jsonb not null default '[]'::jsonb;

alter table public.stores
drop constraint if exists stores_custom_policies_shape_check;

alter table public.stores
add constraint stores_custom_policies_shape_check
check (
  jsonb_typeof(custom_policies) = 'array'
  and jsonb_array_length(custom_policies) <= 4
);

comment on column public.stores.custom_policies is
'Seller-managed storefront policy sections as an array of {title, body} objects. Limited to four sections for Store Setup V1.';
...
'Seller/admin RPC for updating only seller-editable store settings, including custom policy sections. It intentionally does not accept ownership, store_status, admin hold/suspension, billing/provider, audit, or system fields.';
