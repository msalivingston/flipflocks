-- Group 66B follow-up: qualify seller manual order digest call
--
-- The prior replacement added extensions to search_path, but the active RPC
-- still used an unqualified pgcrypto hash call with an untyped sha256 literal.
-- Supabase/Postgres resolved that as the wrong argument signature and failed.
