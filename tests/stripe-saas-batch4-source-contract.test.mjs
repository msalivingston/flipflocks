...
test("catalog utility is read-only toward Stripe and defaults to dry-run", async () => {
  ...
  assert.match(utility, /stripe\.accounts\.retrieve\(\)/);
...
test("no onboarding, entitlement, Pay at Pickup, refund, or Connect application file changed", async () => {
  ...
  const allowed = [
    ...
    "supabase/migrations/20260801105000_trusted_saas_price_catalog_registration.sql",
    "supabase/tests/trusted_saas_price_catalog_registration_test.sql",
    "tests/stripe-saas-runtime.test.mjs",
    "tests/stripe-saas-batch4-source-contract.test.mjs",
    "tests/saas-billing-authority-foundation.test.mjs",
    "tests/requested-only-onboarding-transition.test.mjs",
    "tests/saas-invoice-authority.test.mjs",
  ];
